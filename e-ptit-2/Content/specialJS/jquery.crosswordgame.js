...
(function( $, undefined ) {
    ...
    $.widget("ryanf.crosswordwidget", $.ui.mouse, {
			...
			_create: function () {
			    debugger;
                //member variables
                this.model = GameWidgetHelper.prepGrid(this.options.gridWidth, this.options.gridHeight, this.options.wordObjlist, this.options.otherCells, this.options.isSeparateHints, this.options.hintsIn2Columns)
...
function Model() {
    ...
    this.init = function (grid, list, words, isSeparateHints, hintsIn2Columns) {
        debugger;
        this.grid = grid;
        this.wordObjectList = list;
        this.wordList = words;
        this.isSeparateHints = isSeparateHints;
        this.hintsIn2Columns = hintsIn2Columns
...
var GameWidgetHelper = {
    prepGrid: function (width, height, wordObjectList, otherCells, isSeparateHints, hintsIn2Columns) {
        debugger;
		var grid = new Grid();
		...
		var model = new Model();
		debugger;
        model.init(grid, wordObjectList, wordList, isSeparateHints, hintsIn2Columns);
		grid.fillGrid();
		return model;
    ...
    renderGame: function (container, model) {
            ...
            for (var j=0;j<grid.width();j++) {
				...
				else
				{
					if(cells[i][j].word)
					{
					    ...
					    puzzleGrid += '<td class="' + cells[i][j].css + '"><input type="text" data-letter-id="' + cells[i][j].value + '" maxlength="1" onfocus="VirtualKeyboard.attachInput(this)" onblur="validateLetter(this)"';
        ...
        debugger
        if (!model.isSeparateHints) {
            var words = "<div id='rf-wordcontainer'><ul>"
            $(model.wordList.words).each(function () {
                var leText = this.originalCaseValue;
                if (this.hint)
                    leText = this.hint;
                words += '<li class="rf-ptrue" data-word="' + this.originalValue + '">' + leText + '</li>';
            });
            words += "</ul></div>";
        ...
        {
            var words = "";
            if (model.hintsIn2Columns) {
                words += "<div class='row' id='rf-wordcontainer'>";
                words += "<div class='col-md-6'>Հորիզոնական<ul>";
            }
            else {
                words += "<div id='rf-wordcontainer'>Հորիզոնական<ul>";
            }
            $(model.wordList.words).each(function () {
                if (this.dir == 0) {
                    var leText = this.originalCaseValue;
            ...
            if (model.hintsIn2Columns) {
                words += "</ul></div><div class='col-md-1'></div><div class='col-md-5'> Ուղղահայեաց<ul>";
            }
            else {
                words += "</ul><br\> Ուղղահայեաց<ul>";
            }
