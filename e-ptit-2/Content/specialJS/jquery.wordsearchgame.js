...
(function( $, undefined ) {
    ...
    $.widget("ryanf.wordsearchwidget", $.ui.mouse, {
            ...
            _create : function () {
                //member variables
                this.model      = GameWidgetHelper.prepGrid(this.options.gridWidth, this.options.gridHeight, this.options.wordObjlist, this.options.targetLetters)
...
function Grid() {
    ...
    this.initializeGrid= function(width, height, targetLetters) {
    ...
    for (var i=0;i<this.height();i++) {
        for (var j=0;j<this.width();j++) {
            if (this.cells[i][j].isUnwritten()) {
                ...
                this.cells[i][j].value = this.targetLetters[letterIndex++];
            }
        }
    }
...
//Create a Horizontal Reverse Populator Strategy 
function HorizontalReversePopulator(row, col, word, grid) {
    
    this.grid = grid;
    this.row =  row;
    this.col = col;
    this.word = word;
	this.revWord = new Word(word.reverse());
    this.width = this.grid.width();
    this.height = this.grid.height();
    this.cells = this.grid.cells;
...
//Create a Vertical Reverse Populator Strategy 
function VerticalReversePopulator(row, col, word, grid) {
...
function RightDiagonalPopulator(row, col, word, grid) {
    ...
    this.findContigousSpace = function (xrow, xcol,word) {
        ...
        while (lrow > 0 && lcol < this.width-1) {
            lrow--;
            lcol++;
        }
        var i=0;
...
function Model() {
    ...
    this.init = function(grid,list, words) {
        this.grid = grid;
...
function WordList() {
	...
	this.loadWordsFromArray = function(wordsObjArray){
		...
		for(var i=0;i<wordsObjArray.length;i++)
			$n.push(new Word(wordsObjArray[i].word));
	}
...
var GameWidgetHelper = {
	prepGrid : function (width, height, wordObjectList, targetLetters) {
		var grid = new Grid();
		grid.initializeGrid(width, height, targetLetters);

		var wordList = new WordList();
		wordList.loadWordsFromArray(wordObjectList);

		var model = new Model();
		model.init(grid, wordObjectList, wordList);
		grid.fillGrid();
		return model;
    ...
    renderGame : function(container, model) {
        ...
        var words = "<div id='rf-wordcontainer'><ul class='rf-wc'>"
        $(model.wordList.words).each(function () {
            words += '<li class=rf-ptrue>'+this.originalValue+'</li>';
        });
        words += "</ul></div>";
